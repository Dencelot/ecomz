...
const Clickeds = ({data}:{data:any}) => {
   ...
   const handleClick = (id:number) =>{
      dispatch(addFilters({...filters,
         colors: constructorData.filters.colors.list.find(el=>el.id === id)
      }))
   }
   return (
      <div className='flex items-center flex-wrap mx-[-8px] mt-[18px]'>
      {
         data.list.map((el:any)=>
            <button onClick={()=>handleClick(el.id)}  key={el.id} className={` m-[8px]`}>
