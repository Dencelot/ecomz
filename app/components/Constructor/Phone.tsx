...
const Phone = () => {
   ...
   return (
      <div  className={`fontF${filters.fonts.code} xs:ml-0 ml-[80px] xl:px-[56px] px-[20px] flex-none xl:w-[510px] w-[340px] relative text-[var(--bazalt-100)]`}>
         <div className='absolute z-0 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full bgPhone aspect-square h-full'>
            ...
            <defs>
               <radialGradient id="paint0_radial_9908_230067" cx="0" cy="0" r="1" gradientUnits="userSpaceOnUse" gradientTransform="translate(508 507.5) rotate(90) scale(507.5)">
                  <stop stopColor={filters.colors.text} />
                  <stop offset="0.270833" stopColor={filters.colors.text} stopOpacity="0.6" />
                  <stop offset="0.572917" stopColor={filters.colors.text} stopOpacity="0.2" />
                  <stop offset="1" stopColor={filters.colors.text} stopOpacity="0" />
               </radialGradient>
            </defs>
            </svg>
         </div>
         <div className=' absolute z-[1] xl:left-[56px] xl:right-[56px] left-[20px] right-[20px]'>
            <Image priority={true} className='h-auto w-full' quality={100} alt="phone" src={"/img/construcor/phone.png"} width={800} height={1625}/>
         </div>
         ...
         <div className={`relative z-[2] xl:px-[40px] px-[30px]  xl:py-[80px] py-[50px] `}>
            <div className='xl:h-[606px] h-[466px] overflow-auto phoneScroll'>
               ...
               >
                  {
                     constructorData.phone.smallBanners.map(el=>(
                        <SwiperSlide key={el.id} className=' aspect-square '>
               ...
               >
                  {
                     constructorData.phone.catalog[filters.types.id].map(el=>(
                        <SwiperSlide key={el.id} className='' style={{height:'auto'}}>
                           <article className='flex flex-col h-full'>
                              <div className='mb-[10px] overflow-hidden rounded-[2px] relative  flex items-center justify-center'>
                                 ...
                                 <Image className='absolute z-[1] top-[10px] right-[10px] hoverFilter' quality={100} alt={'избранное'} src={'/img/construcor/favourites.png'} width={36} height={36}/>
                                 <Image className='w-full max-h-[136px] object-contain ' quality={100} alt={el.text} src={el.img} width={134} height={126}/>
                              </div>
                              ...
                              <button style={{background:`${filters?.colors?.text}`}} className='hoverFilter font-[400] text-[12px] text-[var(--white-100)] text-center w-full rounded-[5px] flex items-center justify-center min-h-[30px]'>
                                    {el.buy}
                              </button>
