...
const Menu = ({active,setActive}:{setActive:any,active:boolean}) => {
   const modals = useAppSelector(actualModals)
   return (
      <nav className={` ${active?'translate-x-0 ':' md:translate-x-0 translate-x-full'} transition-transform':''} md:bg-inherit bg-[var(--black-07)] md:h-auto h-screen md:relative absolute top-full right-0 md:top-auto`}>
