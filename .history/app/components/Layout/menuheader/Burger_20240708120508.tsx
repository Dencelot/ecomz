...
const Burger = ({active,mob}:{mob?:boolean,active:boolean}) => {
   ...
   return (
      <span className='flex items-center justify-between flex-col w-[24px] h-[12px]'>
         <span className={`${(modals.demo.open || modals.contacts || active )?`${activeFilter ? 'sm:bg-[var(--white-100)]' : ''} bg-[var(--sunrise-100)] rotate-45 translate-y-[6px]`:`${mob?'bg-[var(--white-100)]':'bg-[var(--bazalt-100)]'}`} transition-transform h-[2px] w-full rounded-[16px] `}>
