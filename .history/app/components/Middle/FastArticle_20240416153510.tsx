...
const FastArticle = ({item}:{item:any}) => {
  ...
  return (
   <article className='art  p-[40px] relative'>
      ...
      <div className={`grid overflow-hidden transition-all ${active?'grid-rows-[1fr]':'grid-rows-[1fr]'}`}>
         <p ref= className={ ` ${active?'':'truncateP'} block min-h-0 `} dangerouslySetInnerHTML={createMarkup(item.descr)}>
         </p>
      </div>
