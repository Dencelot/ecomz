...
const FastArticle = ({item}:{item:any}) => {
  ...
  return (
   <article onClick={handleClick} className='art  p-[40px] relative cursor-pointer'>
      ...
      {!loading&&<p className={ `${active?'opacity-0 absolute bottom-0 delay-1000':''} transition-all truncateP block min-h-0 `} dangerouslySetInnerHTML={createMarkup(item.descr)}>
      </p>}
