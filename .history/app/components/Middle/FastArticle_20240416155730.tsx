...
const FastArticle = ({item}:{item:any}) => {
  ...
  return (
   <article onClick={handleClick} className='art  p-[40px] relative cursor-pointer h-[258px]'>
      <button className={`transition-transform ${active&&'rotate-45'} absolute top-[20px] r`}>
      ...
      {(!active&&!loading)&&<p className={ `transition-all truncateP block`} dangerouslySetInnerHTML={createMarkup(item.descr)}>
      </p>}
