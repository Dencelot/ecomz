      ...
      <div className='flex items-center flex-wrap m-[-8px]'>
      {
         data.list.map((el:any)=>
            <button  key={el.id} className={` p-[8px]`}>
               <span className={`transition-all hover:border-[var(--sunset-08)] hover:border-[2px] w-[32px] h-[32px] p-[4px] flex items-center justify-center rounded-full border-solid border-[1px] border-[var(--black-80)]`}>
                  <span className={`bg-[var(--white-100)] block w-1/2 h-full  rounded-tl-full rounded-bl-full`}>

                  </span>
                  <span style={{backgroundel.text}} className={` block w-1/2 h-full  rounded-tr-full rounded-br-full`}>
