      ...
      <div className=''>
         ...
         <ul  className={`py-[4px] px-[16px] rounded-[12px] bg-[var(--black-80)]`}>
            {constructorD.filters[data.type].list.map((el:any)=>(
               <li onClick={()=>handleClick(el.id)} key={el.id} className={`flex items-center jus transition-colors hover:text-[var(--white-100)] ${filters[data.type].id === el.id ? '' : 'text-[var(--white-06)]'} py-[10px]  border-0 border-solid border-b-[1px] border-b-[--white-02] font-[300] cursor-pointer`}>
                  {el.text}
