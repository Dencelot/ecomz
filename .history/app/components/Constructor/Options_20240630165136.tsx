...
const Options = ({data}:{data:any}) => {
   ...
   return (
      <div className=''>
         ...
         <ul onClick={()=>handleClick(el.id)} className={`py-[4px] px-[16px] rounded-[12px] bg-[var(--black-80)]`}>
            {constructorD.filters[data.type].list.map((el:any)=>(
               <li key={el.id} className={`transition-colors hover:text-[var(--white-100)] text-[var(--white-06)] py-[10px]  border-0 border-solid border-b-[1px] border-b-[--white-02] font-[300] cursor-pointer`}>
                  {el.text}
