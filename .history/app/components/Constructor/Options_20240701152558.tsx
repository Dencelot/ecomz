...
const Options = ({data}:{data:any}) => {
   ...
   return (
      <div className='mb-[35px] '>
         <button onClick={()=>setActive(!active)} className={`py-[10px] mb-[5px] ${active?'text-[var(--white-100)]':''} transition-colors font-[300] border-0 border-solid border-b-[1px] border-b-[--white-02] text-[var(--white-06)] flex items-center justify-between w-full`}>
            {filters[data.type].text}
            <Image className={`ml-2 ${active?'rotate-180':''} transition-transform`} width={25} height={25} alt="arrow" src="/img/construcor/arrow.png"/>
         </button>
         <div className={`grid overflow-hidden ${active?'grid-rows-[1fr]':'grid-rows-[0fr]'} transition-all`}>
            <ul className={`min-h-0 ${active?'py-[4px]':' opacity-0'} transition-opacity   px-[16px] rounded-[12px] bg-[var(--black-80)]`}>
