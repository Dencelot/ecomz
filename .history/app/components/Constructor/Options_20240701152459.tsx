...
const Options = ({data}:{data:any}) => {
   ...
   return (
      <div className='mb-[35px] '>
         <button onClick={()=>setActive(!active)} className='py-[10px] mb-[5px]  font-[300] border-0 border-solid border-b-[1px] border-b-[--white-02] text-[var(--white-06)] flex items-center justify-between w-full'>
            {filters[data.type].text}
            <Image className={`ml-2 ${active?'py-[4px]':' opacity-0'} transition-transform`} width={25} height={25} alt="arrow" src="/img/construcor/arrow.png"/>
         </button>
